...
if (isset($string) && $id && $br) {
    ...
    $part = explode('.',$string);
    $lokFolder = $common->locationslika($id);
    ...
    $srednja_slika = DCROOT.$lokFolder.'/'.$fileName . '_srednja.' . $ext;
     ...
     if (is_file($apLdSl)) {
         ...
         try {
             ...
             $db->where ('IdArtikliSlike', $br);
             if($db->delete('artiklislike')) {

                 // brisemo sliku
                 unlink($apLdSl);

                 if (is_file($mala_slika)) {  unlink($mala_slika); }
                 if (is_file($srednja_slika)) {  unlink($srednja_slika); }
