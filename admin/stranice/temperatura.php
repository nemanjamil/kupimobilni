<div class="col-md-6 col-xs-6">
    <div class="widget box">
        <div class="widget-header">
            <h4><i class="icon-list-alt"></i>Temperatura</h4>
        ...
        <div class="widget-content">
           ...
           /* $cols = Array("K.ImeKulture","ST.senzorTipIme", "LSU.ImeLokSamo", "KL.NazivKulturaLokacija", "SKL.NazivKulLokPod", "PKL.*");
            ...
            $cols = Array("PN.idPodatka","PN.idNotInc","PN.vecaManjaNoti", "STMP.vredSenzorTemp", "PN.vremeNotifikacije","PN.tipNotifikacije", "TN.OpisNotifikacije","PN.poslatMail");
            $db->join("senzortip ST", "ST.IdSenzorTip = PN.tipSenzora");
            $db->join("listasenzora LS", "LS.SenzorSifra = PN.notSifraSenzora");
            $db->join("senzortemp STMP", "STMP.idSenTempIncr  = PN.idPodatka");
            ...
            $db->where("ST.IdSenzorTip", $temperaId);
            ...
            <table class="table table-hover">
                ...
                <tbody>
                    ...
                    foreach ($link as $k=> $v):
                        ...
                        $idNotInc = $v['idNotInc'];
                        $vredSenzorTemp = $v['vredSenzorTemp'];
            ...
            <table
                ...
                $db->where("SKL.IdTipKulTipLok", 2); // temperatura 2
                $link = $db->get("listasenzora LS", null, $cols);
            ...
            <table
                ...
                $limit = 5;
                $cols = Array("ST.vredSenzorTemp", "ST.vremeSenzorTemp");
                $db->orderBy("vremeSenzorTemp", "DESC");;
                $data = $db->get("senzortemp  ST", $limit, $cols);

                $tab = ''; // resetujemo varijablu
                $i = 1;
                foreach ($data as $sds => $link) {
                    $vredSenzorTemp = $link['vredSenzorTemp'];
                    $vremeSenzorTemp = $link['vremeSenzorTemp'];
                    // <td>' . $i . '</td>
                    ...
                    <td>' . $vredSenzorTemp . '</td>
                    <td>' . $vremeSenzorTemp . '</td>
                    <td class="align-center">
                         <span class="btn-group"> <a data-original-title="Search"
                                                                         target="_blank" href="/admin/str/editpodkulturesve/' . $IdPodaciKulTipLok . '"
                                                                         class="btn btn-xs bs-tooltip" title=""><i
