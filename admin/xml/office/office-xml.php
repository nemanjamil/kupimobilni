...
$log->lwrite('OK OFFICE XML -> tip HOST '.$documentroot.'; ROOTLOC : '.ROOTLOC);

$kojijevendor = 8;  // id u tabeli cronzaxml
$vendor = 122;  // u tabeli komitenti
$codetip = 'codeoffice'; // koji je red za sifru u tabel artikli
$codetipLink = ''; // art za ubaciti link na AGRO
$codetipLinkDodatna = ''; // kolona na Dodatnoj opremi
$brend_code = 49; // tabela brendovi Razno
...
if ($kolikoimachild > $BrojDokle) {
    foreach ($dataset as $row) {
        ...
        if ($i >= $BrojDokle) {
            ...
            $MenjanArtikal .= $idArti0 . ' /('.$sifra.') [ '.$ArtikalIdDodatna.' ] ; ';
        ...
        //echo '<br/><br/>';
        if ($i == ($BrojDokle + 1)) break; //KOLIKOXML
        //echo '<hr/>';
        //usleep(100000); // milion je 1 sec

    }
} else {
    $log->lwrite('DOSAO DO 0 OFFICE XML');
    echo 'Nema vise';
    die;
}
...
$log->lwrite('KRAJ OFFICE XML '.$MenjanArtikal);
