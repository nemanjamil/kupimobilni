...
$log->lwrite('OK VERMAX MAKITA ORG -> tip HOST '.$documentroot.'; ROOTLOC : '.ROOTLOC);

$kojijevendor = 10;  // id u tabeli cronzaxml
...
$doc = new DOMDocument();
$doc->load($documentrootAdmin . '/xml/'.$folder.'/xmlovi/vermax_mak.xml');
...
if ($kolikoimachild > $BrojDokle) {
    foreach ($dataset as $row) {
        ...
        //echo '<br/><br/>';
        if ($i == ($BrojDokle + KOLIKOXML)) break; // KOLIKOXML
        //echo '<hr/>';
        //usleep(100000); // milion je 1 sec

    }
} else {
    $log->lwrite('DOSAO DO 0 VERMAX MAK XML');
    echo 'Nema vise';
    die;
}
...
$log->lwrite('KRAJ VERMAX MAKITA XML '.$MenjanArtikal);
